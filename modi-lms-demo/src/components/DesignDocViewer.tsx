...
export function DesignDocViewer({ course, mode = 'doc', onClose, onStart }: {
  ...
  return (
    <div onClick={onClose} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(31,29,29,.54)', backdropFilter: 'blur(5px)', display: 'grid', placeItems: 'center', padding: 15 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(940px, 100%)', maxHeight: 'calc(100vh - 30px)', display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: 24, boxShadow: '0 28px 90px rgba(22,21,21,.24)', overflow: 'hidden', fontFamily: t.font }}>
        <header style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 20, padding: '26px 30px 22px', borderBottom: `1px solid ${t.line}` }}>
          ...
          <button onClick={onClose} aria-label="닫기" style={{ width: 40, height: 40, display: 'grid', placeItems: 'center', border: 0, borderRadius: '50%', background: t.soft, color: t.muted, fontSize: 24, cursor: 'pointer', flex: '0 0 auto' }}>×</button>
        </header>

        <div style={{ padding: '28px 30px 34px', overflowY: 'auto' }}>
          {!p ? (
            <div style={{ padding: '48px 0', textAlign: 'center', color: t.muted, lineHeight: 1.8 }}>
              이 차시의 설계문서는 아직 준비 중이에요.<br />
              <span style={{ fontSize: 13 }}>강의안 스키마가 연결되면 목표 · 성취기준 · 평가가 여기에 자동으로 채워집니다.</span>
            </div>
          ) : (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                {p.rubric && (
                  <Section title="평가 루브릭" full>
                    <div style={{ display: 'grid', gap: 8 }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 8, fontSize: 11, fontWeight: 800, color: t.muted, padding: '0 4px' }}>
                        <span>평가 요소</span><span>기초</span><span>보통</span><span>우수</span>
                      </div>
                      {p.rubric.map((r, i) => (
                        <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 8, padding: '12px 13px', background: '#fff', border: `1px solid ${t.line}`, borderRadius: 12, fontSize: 12.5, lineHeight: 1.55, color: t.inkSoft }}>
