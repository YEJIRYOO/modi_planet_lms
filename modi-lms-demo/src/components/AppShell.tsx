...
export function AppShell() {
  ...
  const navItem = (to: string, label: string, icon: string, end?: boolean) => (
    <NavLink className="app-shell__nav-item" to={to} end={end} aria-label={label} style={({ isActive }) => ({
      ...
      <span style={{ width: 20, flex: '0 0 20px', textAlign: 'center', fontSize: 16 }}>{icon}</span>
      <span className="app-shell__nav-label">{label}</span>
    </NavLink>
  );

  return (
    <div className="app-shell" style={{ fontFamily: t.font, color: t.ink, background: t.soft, minHeight: '100vh' }}>
      <aside className="app-shell__sidebar" style={{ position: 'fixed', inset: '0 auto 0 0', width: 244, display: 'flex', flexDirection: 'column', padding: '22px 16px 16px', background: t.soft, borderRight: `1px solid ${t.line}`, zIndex: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '2px 6px 18px' }}>
          <span className="app-shell__brand" style={{ fontWeight: 820, fontSize: 20, letterSpacing: '-.03em', color: t.ink }}>MODI Planet</span>
        ...
        <div style={{ display: 'grid', gap: 4 }}>
          ...
          {navItem('/code-editor', '코드 에디터', '⌨')}
          {navItem('/ai-lab', 'AI LAB', '✦')}
        </div>
      ...
      <div className="app-shell__main" style={{ marginLeft: 244, background: t.surface, minHeight: '100vh' }}>
