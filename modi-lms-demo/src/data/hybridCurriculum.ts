...
const CLASSROOM_GARDEN: HybridCurriculum = {
  ...
  examples: [
    '교실 환경을 실제로 관찰하며 씨앗부터 꽃까지 키우는 Classroom Garden을 만들고 싶습니다. ENV 센서로 온도 20~26°C, 습도 40~70%, 밝기 40~82%의 적정 범위를 측정하고 토양 수분은 35~78%를 유지하게 해주세요. 다이얼 값에 따라 한 번에 줄 물의 양을 5~27ml로 정하고 버튼을 누르는 순간에만 물을 주며, ToF 센서 12cm 안으로 손을 가져오면 쓰다듬기로 행복도를 높이되 18cm 밖으로 뺐다가 다시 가까이 해야 다음 입력이 되게 해주세요. 환경·토양·행복의 균형에 따라 씨앗, 새싹, 잎, 봉오리, 꽃의 5단계로 성장시키고 LED로 건강·갈증·과습·어두움 상태를 표시하며 35분 수업 모드와 4분 체험 모드를 제공하고 싶습니다.',
  ],
  keywords: [
    {
      label: '교실 환경',
...
const EMBER_AND_TIDE: HybridCurriculum = {
  ...
  examples: [
    '불의 아이 EMBER와 물의 아이 TIDE를 번갈아 조작해 세 개의 유적을 탈출하는 물불 퍼즐 어드벤처를 만들고 싶습니다. IMU를 좌우로 기울여 현재 캐릭터를 이동시키고 버튼을 한 번 누르면 점프, 더블클릭하면 캐릭터 전환이 되게 하며 서로 다른 원소 웅덩이와 포털 규칙을 적용해주세요. ENV 센서를 손으로 데워 얼음을 녹이고 조도를 낮춰 빛의 문을 잠재우며, 조이스틱으로 위·오른쪽·왼쪽 같은 룬과 거울 방향 순서를 입력하게 해주세요. 빙결 금고, 일식 도서관, 공명 코어의 세 스테이지를 순서대로 진행하고 LED는 활성 원소와 위험·성공 상태를 표시하며 스피커는 점프·정답·완료 효과음과 마지막 멜로디를 연주하게 해주세요.',
  ],
  keywords: [
    {
      label: '두 캐릭터 조작',
